@component('components.modal_medium')
	...
	@slot('body')
 	...
 	<div class="row">
        ...
        <form action="{{ route('cases.addnoveltyhasdata') }}" method="POST" id="myformEditNoveltyHas">
        <input type="hidden" name="component" id="component_has" value="case_has">
    {{ csrf_field() }}

    <div class="form-group">
        <!-- Novedad (question_id) -->
        <label for="novelty" class="strong">Novedad</label>
        <select class="form-control form-control-sm input_data" id="types_category_novelty_has_edit" name="data[0][question_id]">
        ...
        <select class="form-control form-control-sm input_data" id="state_novelty_has_edit" name="data[0][options][0][option_id]">
            <option value="view_all">Seleccione...</option>
        ...
        <input type="hidden" id="value_novelty_has_edit" name="data[0][options][0][value]">
    </div>
