...
eventTimeFormat: {
      ...
      eventClick: function(info) {
        ...
        $("#fecha_evento_diary").val(moment(info.event.start).format('YYYY-MM-DD')); //campo fecha
        $('#input-date-picker1').val(moment(info.event.start).format('LT')); //campo inicio
        $("#input-date-picker1").prop( "disabled", false );//campo inicio
        $('#input-date-picker2').val(moment(info.event.end).format('LT')); //campo fin
        $('#title_diary').val(info.event.title); //campo titulo
        ...
        $.ajax({
            ...
            success: function(res)
            {
              ...
              if (res.userowner) {
                $("#fecha_evento_diary").val(moment(res.inicio).format('YYYY-MM-DD')); //campo fecha
                $('#input-date-picker1').val(moment(res.inicio).format('LT')); //campo inicio
                $('#input-date-picker2').val(moment(res.fin).format('LT')); //campo fin
                ...
                $("#fecha_evento_diary_show").val(moment(info.event.start).format('YYYY-MM-DD')); //campo fecha
                $('#input-date-picker1_show').val(moment(res.inicio).format('LT')); //campo inicio
                $('#arf_title_url').attr("href",res.url).text(res.url);
                $('#input-date-picker2_show').val(moment(res.fin).format('LT')); //campo fin
                $('#title_diary_show').val(res.title); //campo titulo
  ...
  dateClick: function(info) {
    ...
    $('#diary_id').val('0');
    $("#fecha_evento_diary").val(moment(info.date).format('YYYY-MM-DD')); //campo fecha
    $("#fecha_evento_diary").prop( "disabled", true );//campo fecha
    var hora = info.date.getHours();
    if (hora != 0) { 
      $('#input-date-picker1').val(moment(info.date).format('LT'));//campo inicio
...
$("#myformCreateDiary").on('submit',function(e){
    ...
    //toma datos del formulario para crear o actulizar el evento en tiempo real
      ...
      start= moment(fecha+' '+start).format('YYYY-MM-DD HH:mm:ss');
